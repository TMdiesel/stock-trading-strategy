...
class Config:
    ...
    filelist = [pathlib.Path("./data/union.dat"), pathlib.Path("./data/divide.dat")]
...
def dat_to_dict(filename: pathlib.Path) -> t.Dict:
    ...
    if filename.stem == "union":
        replaced_data_list = [
            [field.replace("\n", "").replace("株→1株", "") for field in data]
            for data in splited_data_list
        ]
        data_dict = [
            {
                "date_of_right_allotment": data[4].replace("/", "-"),
                "code": int(data[1]),
                "name": data[2],
                "before": float(data[3]),
                "after": 1,
            }
            for data in replaced_data_list
        ]
    elif filename.stem == "divide":
        replaced_data_list = [
            [field.replace("\n", "").replace("1：", "") for field in data]
            for data in splited_data_list
        ]
        data_dict = [
            {
                "date_of_right_allotment": data[6].replace("/", "-"),
                "code": int(data[1]),
                "name": data[2],
                "before": 1,
                "after": float(data[3]),
...
def insert_data(
    table: sqlalchemy.Table, engine: sqlalchemy.engine, data_dict: t.Dict
) -> None:
    """dataをinsert"""
...
def create_table(
    meta: sqlalchemy.MetaData, engine: sqlalchemy.engine, table_name: str
) -> None:
    """table作成"""
    table = Table(
        table_name,
