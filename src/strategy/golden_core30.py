...
def create_stock_full_data(db_file_name, code_list, start_date, end_date):
    """
    指定した銘柄(code_list)それぞれの単元株数と始値、終値、高値、低値、出来高を含む辞書を作成
    """
    stocks = {}
    tse_index = sim.tse_date_range(start_date, end_date)
    conn = sqlite3.connect(db_file_name)
    for code in code_list:
        unit = conn.execute('SELECT unit from brands WHERE code = ?',
                            (code,)).fetchone()[0]
        prices = pd.read_sql('SELECT date, open, high,low,close,volume '
                             'FROM prices '
                             'WHERE code = ? AND date BETWEEN ? AND ?'
                             'ORDER BY date',
                             conn,
...
def simulate_golden_dead_cross(db_file_name,
    ...
    def get_close_price_func(date, code):
        return stocks[code]['prices']['close'][str(date)]
